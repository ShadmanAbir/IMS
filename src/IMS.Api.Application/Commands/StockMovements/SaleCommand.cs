...
/// <summary>
/// Command to record a sale (stock decrease)
/// </summary>
public class SaleCommand : ICommand<Result>
{
...
public class SaleCommandValidator : AbstractValidator<SaleCommand>
{
    public SaleCommandValidator()
    {
        ...
        RuleFor(x => x.Quantity)
            .GreaterThan(0)
            .WithMessage("Sale quantity must be positive");
...
/// <summary>
/// Handler for SaleCommand
/// </summary>
public class SaleCommandHandler : ICommandHandler<SaleCommand, Result>
{
    private readonly IInventoryRepository _inventoryRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SaleCommandHandler(IInventoryRepository inventoryRepository, IUnitOfWork unitOfWork)
    {
        _inventoryRepository = inventoryRepository;
        _unitOfWork = unitOfWork;
    }

    public async Task<Result> Handle(SaleCommand request, CancellationToken cancellationToken)
    {
        ...
        if (inventoryItem.AvailableStock < request.Quantity && !inventoryItem.AllowNegativeStock)
        {
            return Result.Failure("INSUFFICIENT_STOCK", 
                $"Insufficient stock. Available: {inventoryItem.AvailableStock}, Requested: {request.Quantity}");
        }
        ...
        {
            return Result.Failure("OUT_OF_STOCK", ex.Message);
        }
